...
| # | Task | Notes |
|---|------|-------|
| F0 | RGW model loading in serving | `load_model()` currently pulls from HuggingFace Hub on every pod start. The original RGW path was removed when the self-trained weights were replaced by the TIAToolbox pretrained model (pcam-kaggle-001 weights showed ~0.37–0.41 flat probabilities — see `registry/experiments/pcam-kaggle-001.yaml`). Once a well-trained model is pushed to RGW, restore `load_model()` to: (1) read `RGW_ENDPOINT`, `RGW_ACCESS_KEY`, `RGW_SECRET_KEY` from env; (2) use boto3 to download the weights from the configured bucket/key; (3) load into the model skeleton via `torch.load`. The Helm chart, Sealed Secret, and ConfigMap are already wired — only `load_model()` needs updating. |
